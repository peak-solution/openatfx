...
/**
 * Test case for <code>com.peaksolution.openatfx.io.AtfxWriterTest</code>.
 * 
...
//            aoSession.commitTransaction();
    ...
    void testWriteXML2() {
        try {
            URL url = AoSessionImplTest.class.getResource("/com/peaksolution/openatfx/test.atfx");
            aoSession = AoServiceFactory.getInstance().newAoFactory(orb)
                                        .newSession("FILENAME=" + new File(url.getFile()));
    ...
    @Test
    void testWriteInstanceAttributesToXML() throws IOException {
        try {
            Path atfxFile = Files.createFile(exportFolder.resolve("unit_test.atfx"));
            aoSession = AoServiceFactory.getInstance().newEmptyAoSession(orb, atfxFile.toFile(), "asam35");
