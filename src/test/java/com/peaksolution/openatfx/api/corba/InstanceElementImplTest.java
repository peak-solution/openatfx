...
/**
 * Test case for <code>com.peaksolution.openatfx.InstanceElement</code>.
 * 
...
public class InstanceElementImplTest {
    ...
    void testSetValue() {
        try {
            ...
            assertThat(ieLc.getValue("values").value.u.floatSeq()).containsExactly(new float[] { -1, 0, 1, 9999999 });
//            assertEquals(true,
//                         Arrays.equals(new float[] { -1, 0, 1, 9999999 }, ieLc.getValue("values").value.u.floatSeq()));
            ...
            // instance attribute
            ieDts.addInstanceAttribute(ODSHelper.createStringNVU("instattr", "test"));
            assertThat(ieDts.getValue("instattr").value.u.stringVal()).isEqualTo("test");
            ieDts.setValue(ODSHelper.createStringNVU("instattr", "aaa"));
            assertThat(ieDts.getValue("instattr").value.u.stringVal()).isEqualTo("aaa");
            ieDts.removeInstanceAttribute("instattr");
            
            // set and check Blob value
            ApplicationElement aeTstser = aoSession.getApplicationStructure().getElementByName("tstser");
            InstanceElement ieTs = aeTstser.getInstanceById(ODSHelper.asODSLongLong(1));
            assertThat(ieTs.getValue("appl_attr_dt_blob").value.u.blobVal()).isNull();
            Blob blob = aoSession.createBlob();
            blob.setHeader("header");
            blob.append(new byte[] { 1, 2, 3, 4, 5 });
            NameValueUnit blobNvu = ODSHelper.createEmptyNVU("appl_attr_dt_blob", DataType.DT_BLOB);
            blobNvu.value.u.blobVal(blob);
            blobNvu.value.flag = (short) 15;
            ieTs.setValue(blobNvu);
            assertThat(ieTs.getValue("appl_attr_dt_blob").value.u.blobVal().getHeader()).isEqualTo("header");
            assertThat(ieTs.getValue("appl_attr_dt_blob").value.u.blobVal().getLength()).isEqualTo(5);
            ieTs.setValue(ODSHelper.createEmptyNVU("appl_attr_dt_blob", DataType.DT_BLOB));
            assertThat(ieTs.getValue("appl_attr_dt_blob").value.u.blobVal()).isNull();
        } catch (AoException e) {
    ...
    @Test
    void testInstanceAttributeWithUnit() {
        try {
            // add test attributes
    ...
    @Test
    void testRemoveInstanceAttribute() {
        try {
    ...
    @Test
    void testRenameInstanceAttribute() {
        try {
            ieDts.addInstanceAttribute(ODSHelper.createStringNVU("inst_attr", "test"));
    ...
    @Test
    void testListRelatedInstancesByRelationship() {
        try {
    ...
    @Test
    void testGetRelatedInstancesByRelationship() {
        try {
    ...
    void testCreateRelation() {
        try {
            ...
            // check self relations
            ApplicationRelation[] selfRelations = aoSession.getApplicationStructure().getRelations(aeSM, aeSM);
            assertEquals(4, selfRelations.length);
            ApplicationRelation arSMx = null;
            ApplicationRelation arSMy = null;
            for (ApplicationRelation currentRel : selfRelations) {
                if (currentRel.getRelationName().equals("x-axis-for-y-axis")) {
                    assertEquals(1, ieSMy.getRelatedInstances(currentRel, "*").getCount());
                    arSMx = currentRel;
                } else if (currentRel.getRelationName().equals("y-axis-for-x-axis")) {
                    assertEquals(1, ieSMx.getRelatedInstances(currentRel, "*").getCount());
                    arSMy = currentRel;
                }
            }
            // re-set y->x
    ...
    @Test
    void testCreateRelatedInstances() {
        try {
