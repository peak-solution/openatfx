...
class ValueMatrixOnSubMatrixImpl extends ValueMatrixPOA {
    ...
    public String[] listColumns(String colPattern) throws AoException {
        ...
        List<String> list = new ArrayList<>(ies.length);
    ...
    public Column[] getColumns(String colPattern) throws AoException {
        try {
            ...
            List<Column> list = new ArrayList<>(ies.length);
            for (int i = 0; i < ies.length; i++) {
                ColumnImpl columnImpl = new ColumnImpl(this.modelPOA, this.sourceSubMatrix.corbaCache, ies[i], this.mode);
    ...
    public Column[] getIndependentColumns(String colPattern) throws AoException {
        try {
            ...
            for (int i = 0; i < ies.length; i++) {
                NameValueUnit nvu = ies[i].getValueByBaseName("independent");
                if (nvu.value.flag == 15 && nvu.value.u.shortVal() > 0) {
                    ColumnImpl columnImpl = new ColumnImpl(this.modelPOA, this.sourceSubMatrix.corbaCache, ies[i],
                                                           this.mode);
                    Column column = ColumnHelper.unchecked_narrow(modelPOA.servant_to_reference(columnImpl));
                    list.add(column);
                }
    ...
    public TS_ValueSeq getValueVector(Column col, int startPoint, int count) throws AoException {
        ...
        CorbaAtfxCache atfxCache = this.sourceSubMatrix.corbaCache;
        ...
        NameValueUnit globalFlagValue = atfxCache.getInstanceValue(aidLc, iidLc, null, "global_flag");
        if (globalFlagValue.value.flag == 15) {
            valueSeq.flag = new short[count];
            Arrays.fill(valueSeq.flag, globalFlagValue.value.u.shortVal());
        }
        if (valueSeq.flag == null) {
            NameValueUnit flagsValue = atfxCache.getInstanceValue(aidLc, iidLc, null, "flags");
            if (flagsValue.value.flag == 15) {
                valueSeq.flag = new short[count];
                System.arraycopy(flagsValue.value.u.shortSeq(), startPoint, valueSeq.flag, 0, count);
            }
        }
    ...
    private void handleValuesExplicit(NameValueUnit values, TS_ValueSeq valueSeq, DataType targetDt, int startPoint,
        ...
        if (rawDt == DataType.DS_STRING && targetDt == DataType.DT_STRING) {
            ...
            for (int i = 0; i < count; i++) {
                valueSeq.u.dateVal()[i] = values.value.u.stringSeq()[startPoint + i];
            }
    ...
    public NameValueSeqUnit[] getValue(Column[] columns, int startPoint, int count) throws AoException {
        List<NameValueSeqUnit> nvsuList = new ArrayList<>();

        for (Column column : columns) {
            TS_ValueSeq tsValue = getValueVector(column, startPoint, count);
            String unit = column.getUnit();
            nvsuList.add(new NameValueSeqUnit(column.getName(), tsValue, unit == null ? "" : unit));
        }
